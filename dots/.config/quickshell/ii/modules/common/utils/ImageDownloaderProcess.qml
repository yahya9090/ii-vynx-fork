...
Process {
    ...
    running: true
    command: ["bash", "-c",
        `mkdir -p $(dirname '${processFilePath()}'); [ -f '${processFilePath()}' ] || (curl -sSL '${sourceUrl}' -o '${processFilePath()}'${fallbackUrl !== "" ? ` || curl -sSL '${fallbackUrl}' -o '${processFilePath()}'` : ""}) && file '${processFilePath()}'`
    ]
