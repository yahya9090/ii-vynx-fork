...
`, Ai.interfaceRole);
    ...
    ColumnLayout {
            ...
            Rectangle {
                ...
                RowLayout {
                    ...
                    StatusItem {
                        // Chat save indicator
                        visible: Ai.messageIDs.length > 0
                        icon: Ai.namingInProgress ? "progress_activity"
                            : Ai.currentChatMetadata?.title?.length > 1 ? Ai.waitingForResponse ? "hourglass_empty" : "save"
                            : Config.options.ai.autoSave ? "save_clock" : "file_save_off"
                        statusText: ""
                        description: Ai.namingInProgress ? Translation.tr("Auto-naming in progress...")
                            : Ai.currentChatMetadata?.title?.length > 1 ? Ai.waitingForResponse ? Translation.tr("Waiting for response") : Translation.tr("Chat is saved")
                            : Config.options.ai.autoSave ? Translation.tr("Chat is not saved (auto save enabled)\nSave with '/save'\nOr continue writing to automatically save")
                            : Translation.tr("Chat is not saved\nSave with auto-naming using /save\nOr enable auto save in settings")
                    }
                }
            }
        ...
        Rectangle { // Input area
            ...
            radius: Appearance.rounding.normal - root.padding
            color: Appearance.colors.colLayer2
            implicitHeight: Math.max(inputFieldRowLayout.implicitHeight + inputFieldRowLayout.anchors.bottomMargin + commandButtonsRow.implicitHeight + commandButtonsRow.anchors.bottomMargin + spacing, 45) + (attachedFileIndicator.implicitHeight + spacing + attachedFileIndicator.anchors.topMargin)
            clip: true
